...
LoadDatas = function () {
  ...
  if (!Session.get("subscribeds")) {
    Meteor.call('getSubscription', document.cookie, (error, result) => {
      Session.set("subscribeds", result.data.results);
    });
  }

  if (!Session.get("newestForums")) {
    Meteor.call('getNewestForums', document.cookie, (error, result) => {
      Session.set("newestForums", result.data.results);
    });
  }
  if (!Session.get("topForums")) {
    Meteor.call('getTopForums', document.cookie, (error, result) => {
      Session.set("topForums", result.data.results);
    });
  }
  if (!Session.get("allForums")) {
    Meteor.call('getGeneralForums', document.cookie, (error, result) => {
      Session.set("allForums", result.data.results);
    });
  }
}
...
XBBCODE = (function () {
  ...
  tags = {
    ...
    */
    ...
    "mention": {
      openTag: function (params, content) {
        return '<button class="mdc-button mdc-button--dense">@';
      },
      closeTag: function (params, content) {
        return '</button>';
      }
